...
const ICON_PROVIDERS: Record<string, (className: string) => React.ReactNode> = {
  figma: (className) => (
...
export function TechIcon({ name, className = "h-3.5 w-3.5" }: { name: string; className?: string }) {
  ...
  const key = Object.keys(ICON_PROVIDERS).find((k) => normalized.includes(k))

  if (key && ICON_PROVIDERS[key]) {
    return ICON_PROVIDERS[key](className)
  }
