...
import { ArrowUpRight, Code2, ExternalLink, Image as ImageIcon } from "lucide-react"
        ...
        {projects.map((project) => (
          ...
          >
            <div>
              ...
              <div className="mt-5 relative aspect-[16/9] w-full overflow-hidden rounded-lg border border-border bg-secondary/30">
                {project.image ? (
                  <img
                    src={project.image}
                  ...
                  <div className="flex h-full w-full flex-col items-center justify-center gap-2 border border-dashed border-border/80 bg-secondary/20 p-4 text-center">
                    <div className="rounded-full bg-card p-3 text-muted-foreground shadow-2xs border border-border">
                      <ImageIcon className="h-5 w-5 text-brand" />
                    </div>
                    <div className="text-xs">
                      <p className="font-semibold text-foreground">Vista previa del proyecto</p>
                      <p className="text-[11px] text-muted-foreground mt-0.5">Espacio para captura de pantalla (16:9)</p>
                    </div>
                  </div>
                )}
              </div>
